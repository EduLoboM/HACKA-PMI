...
<section class="bg-white border border-slate-200 shadow-sm overflow-hidden">
	<div class="h-9 px-3 flex items-center gap-2 border-b border-slate-200 bg-slate-50/80">
		...
		<span class="ml-auto text-[10px] text-slate-500 hidden lg:block truncate">
		...
		{#if classificacao.posicaoCorte != null}
			<button
				type="button"
				onclick={onIrAoCorte}
				class="shrink-0 inline-flex items-center gap-1 px-2 py-0.5 bg-rose-50 border border-rose-300 text-rose-800 text-[10px] font-bold hover:bg-rose-100 transition-colors duration-150 cursor-pointer font-mono ml-2"
			>
				<span class="i-lucide-scissors text-xs"></span>
				<span>Linha: {classificacao.posicaoCorte}º</span>
			</button>
		{:else}
			<span class="shrink-0 inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-50 border border-emerald-300 text-emerald-800 text-[10px] font-bold font-mono ml-2">
